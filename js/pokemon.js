...
function crossSubtract(hpDict, damageDict, maxHP=-1) {
    ...
    Object.keys(hpDict).sort().forEach((hp, i) => {
        for (let damage in damageDict) {
            ...
            if (i==0) {
                push(resultDamage, d, damageDict[damage]);
            }
        }
    });
...
// HPやダメージに定数を足す
function offsetKeys(dict, v, keepZero=true) {
    let result = {};
    let h, newKey;
    for (let key in dict) {
        h = Number(key)
        if (h == 0 && keepZero) {
            ...
            newKey = String(Math.max(0, h + v));
        }
        if (newKey != '0' && key.slice(-2) == '.0') { newKey += '.0' }
        push(result, newKey, dict[key]);
    }
    return result;
}
// HPやダメージに定数倍する
...
export class Pokemon {
    ...
    static async init() {
        ...
        try {
            ...
            for (let line of lines) {
                ...
                if (name.includes('ロトム')) {
                    ...
                    displayName = displayName.replace('ホワイト','');
                    displayName = displayName.replace('ブラック','');
                }
        ...
        try {
            ...
            for (let name in adoption) {
                ...
                switch (name) {
                    case 'イルカマン(マイティ)':
                        names.push('イルカマン(ナイーブ)');
                        break;
                    case 'コオリッポ(アイス)':
                        names.push('コオリッポ(ナイス)');
                        break;
                    case 'ザシアン(れきせん)':
                        names.push('ザシアン(けんのおう)');
                        break;
                    case 'ザマゼンタ(れきせん)':
                        names.push('ザマゼンタ(たてのおう)');
                        break;
                    case 'テラパゴス(ノーマル)':
                        names.push('テラパゴス(テラスタル)');
                        names.push('テラパゴス(ステラ)');
                        break;
                    case 'メテノ(コア)':
                        names.push('メテノ(りゅうせい)');
                        break;
                    case 'モルペコ(まんぷく)':
                        names.push('モルペコ(はらぺこ)');
                        break;
                }
                for (let s of names) {
    ...
    getMoveType(move, weather='') {
        ...
        if (['テラバースト','テラクラスター'].includes(move) && this.terastal) {
        ...
        if (['さばきのつぶて','めざめるダンス'].includes(move)) {
            return this.type[0]
        }
        if (move == 'レイジングブル') {
            return this.type.slice(-1)[0];
        }
    ...
    // 現在のタイプを返す
    currentType() {
        return (this.terastal && this.Ttype != 'ステラ') ? [this.Ttype] : this.type;
    }
    // 浮いていればTrueを返す
    isFloat() {
        return this.currentType().includes('ひこう') || this.ability == 'ふゆう';
    }
...
export class Battle {
    ...
    defenceAbility(move) {
        ...
        if (!(move in Pokemon.moves) || p2.item == 'とくせいガード' || p2.ability == 'ファントムガード') {
            return p2.ability
        }
        if (['シャドーレイ','フォトンゲイザー','メテオドライブ'].includes(move) ||
            p1.ability in ['かたやぶり','ターボブレイズ','テラボルテージ'] ||
            (p1.ability == 'きんしのちから' && p1.moveClass(move) == '変化')) {
                return ''
            }
        return p2.ability
    }
    ...
    isCritical(move) {
        ...
        //let p2 = (this.attackSide == 0) ? this.pokemon[1] : this.pokemon[0]; // 防御側
        if (this.critical) {
            return !['シェルアーマー','カブトアーマー'].includes(this.defenceAbility(move));
        } else {
            return false;
        }
    }
        ...
        if (moveType == 'ステラ' && p2.terastal) {
            ...
            for (let t of p2_type) {
                ...
                if (['しんがん','きもったま'].includes(p1.ability) && t=='ゴースト' && ['ノーマル','かくとう'].includes(moveType)) {
                    ...
                    if (move == 'フライングプレス') {
                        r *= Pokemon.typeCorrections[Pokemon.typeID['ひこう']][Pokemon.typeID[t]]
                    }
                }
            }
        }
        // テラスシェルはマルチスケイルと同じものとして実装する
        //if (ability2 == 'テラスシェル' && r != 0 && p2.hp == p2.status[0]) {
        //    r = 0.5;
        //    this.note.push(ability2);
        //}
        return r;
    }
    // 威力補正
    powerCorrection(move) {
        ...
        let p1 = (this.attackSide == 0) ? this.pokemon[0] : this.pokemon[1]; // 攻撃側
        let p2 = (this.attackSide == 0) ? this.pokemon[1] : this.pokemon[0]; // 防御側
        const ability2 = this.defenceAbility(move)
        const moveType = p1.getMoveType(move, this.weather);
        ...
        if (['しおふき','ドラゴンエナジー','ふんか'].includes(move)) {
            const rate = int(r*p1.hp/p1.status[0]);
            r = Math.round(r*rate);
            this.correctedPower = Math.trunc(rate*Pokemon.moves[move]['power']);
            this.note.push(`威力${this.correctedPower}`);
        }
        ...
        if (p1.item.includes('しらたま') && p1.hasItem) {
            if (p1.name.includes('パルキア') && ['みず','ドラゴン'].includes(moveType)) {
                r = Math.round(r*4915/4096);
                this.note.push(p1.item);
            }
        }
        if (p1.item == 'こころのしずく' && p1.hasItem) {
            if (['ラティオス','ラティアス'].includes(p1.name) && ['エスパー','ドラゴン'].includes(moveType)) {
                r = Math.round(r*4915/4096);
                this.note.push(p1.item);
            }
        }
        if (p1.item.includes('こんごうだま') && p1.hasItem) {
            if (p1.name.includes('ディアルガ') && ['はがね','ドラゴン'].includes(moveType)) {
                r = Math.round(r*4915/4096);
                this.note.push(p1.item);
            }
        }
        if (p1.item.includes('はっきんだま') && p1.hasItem) {
            if (p1.name.includes('ギラティナ') && ['ゴースト','ドラゴン'].includes(moveType)) {
                r = Math.round(r*4915/4096);
                this.note.push(p1.item);
            }
        }
        ...
        if (Pokemon.fields.includes(this.field)) {
            if (this.field == 'エレキフィールド') {
                if (moveType=='でんき' && !p1.isFloat()) {
                    r = Math.round(r*5325/4096);
                }
                if (move == 'ライジングボルト' && !p2.isFloat()) {
                    r = Math.round(r*2);
                }
            } else if (this.field == 'グラスフィールド') {
                if (moveType == 'くさ' && !p1.isFloat()) {
                    r = Math.round(r*5325/4096);
                }
                if (['じしん','じならし','マグニチュード'].includes(move) && !p2.isFloat()) {
                    r = Math.round(r*0.5);
                }
            } else if (this.field == 'サイコフィールド') {
                if (moveType == 'エスパー' && !p1.isFloat()) {
                    r = Math.round(r*5325/4096);
                }
                if (move == 'ワイドフォース' && !p2.isFloat()) {
                    r = Math.round(r*1.5);
                }
            } else if (this.field == 'ミストフィールド') {
                if (moveType == 'ドラゴン' && !p2.isFloat()) {
                    r = Math.round(r*0.5);
                }
                if (move == 'ミストバースト' && !p1.isFloat()) {
                    r = Math.round(r*1.5);
                }
            }
        }
    ...
    attackCorrection(move) {
        ...
        if (p1.ability == 'しんりょく') {
            if (moveType == 'くさ' && p1.hp/p1.status[0] <= 1/3) {
                r = Math.round(r*1.5);
                this.note.push(p1.ability);
            }
        }
        ...
        if (ability2 == 'わざわいのうつわ') {
            if (moveClass == '特殊') {
                r = Math.round(r*3072/4096);
                this.note.push(ability2);
            }
        }
        if (ability2 == 'わざわいのおふだ') {
            if (moveClass == '物理') {
                r = Math.round(r*3072/4096);
                this.note.push(ability2);
            }
    ...
    defenceCorrection(move) {
        ...
        if (p1.ability == 'わざわいのたま') {
            if (moveClass == '特殊' && !['サイコショック','サイコブレイク'].includes(move)) {
                r = Math.round(r*3072/4096);
                this.note.push(p1.ability);
            }
        }
        ...
        if (p2.ability == 'クォークチャージ') {
            if ((p2.item == 'ブーストエナジー' && p2.hasItem) || this.field == 'エレキフィールド') {
                ...
                if ((moveClass == '物理' || ['サイコショック','サイコブレイク'].includes(move)) && abcds.indexOf(Math.max(...abcds)) == 2) {
                    r = Math.round(r*5325/4096);
                    this.note.push('B上昇');
                } else if (moveClass == '特殊' && !['サイコショック','サイコブレイク'].includes(move) && abcds.indexOf(Math.max(...abcds)) == 4) {
                    r = Math.round(r*5325/4096);
                    this.note.push('D上昇');
                }
            }
        ...
        if (ability2 == 'ふしぎなうろこ') {
            if (Pokemon.ailments.includes(p2.ailment) && moveClass == '物理') {
                r = Math.round(r*1.5);
                this.note.push(ability2);
            }
        }
    ...
    damageCorrection(move) {
        ...
        const moveClass = p1.getMoveClass(move);
        // 技
        if (['アクセルブレイク','イナズマドライブ'].includes(move)) {
            if (this.defenceTypeCorrection(move) > 1) {
                r = Math.round(r*5461/4096);
                this.note.push('弱点強化');
            }
        }
        ...
        if (ability2 == 'こおりのりんぷん') {
            if (moveClass == '特殊') {
                r = Math.round(r*0.5);
                this.note.push(ability2);
            }
        }
        if (['こんがりボディ','もらいび'].includes(ability2)) {
            if (moveType == 'ほのお') {
                r = 0;
                this.note.push(ability2);
            }
        ...
        if (['ちくでん','でんきエンジン','ひらいしん'].includes(ability2)) {
            if (moveType == 'でんき') {
                r = 0;
                this.note.push(ability2);
            }
        }
        if (['ちょすい','よびみず'].includes(ability2)) {
            if (moveType == 'みず') {
                r = 0;
                this.note.push(ability2);
            }
        }
        if (['どしょく','ふゆう'].includes(ability2)) {
            if (moveType == 'じめん') {
                r = 0;
                this.note.push(ability2);
            }
    ...
    calculateDamage(move, powerRatio=1) {
        ...
        rRank = p2.getRankCorrection(ind);
        if (['DDラリアット','せいなるつるぎ','しんぴのつるぎ'].includes(move)) {
            rRank = 1
        }
        if (p1.ability == 'てんねん') {
            if (rRank > 1) {
        ...
        if (['ゆき','すなあらし'].includes(this.weather)  && !['のーてんき','エアロック'].includes(p1.ability) && !['のーてんき','エアロック'].includes(p2.ability)) {
            if (this.weather == 'ゆき' && p2.currentType().includes('こおり') && moveClass == '物理') {
                finalDefence = Math.trunc(finalDefence*1.5);
            } else if (this.weather == 'すなあらし' && p2.currentType().includes('いわ') && moveClass == '特殊') {
                finalDefence = Math.trunc(finalDefence*1.5);
            }
        }
        ...
        for (let i=0; i<16; i++) {
            ...
            push(this.damage, String(damage), 1);
        }
        return true;
    }
    // 致死率・確定数の計算
    lethal(move, hp={}) {
        ...
        if (this.stealthrockDamage) {
            ...
            // きのみ回復
            if (recoveryFruit.includes(p2.item)) {
                hp = p2.fruitRecovery(hp);
            }
        }
        // マルチスケイル
        const ability2 = this.defenceAbility(move);
        let reducible = Pokemon.multiScales.includes(ability2);
        ...
        for (let i=0; i<this.maxRepeat; i++) {
            ...
            if (reduced) { this.note.push(ability2); }
            // 記録
            if (i < this.nCombo) {
            ...
            let d;
            // いのちのたま
            d = this.orbDamage ? Math.trunc(p2.status[0]/10) : 0;
            if (d && i==0) {
                hp = offsetKeys(hp, -d);
                if (i+1 == this.nCombo) {
                    totalConstDamage += d;
            ...
            // ゴツゴツメット (1発目のみ)
            d = (this.metDamage) ? Math.trunc(p2.status[0]/6) : 0;
            if (d && i==0) {
                hp = offsetKeys(hp, -d);
                totalConstDamage += d; 
            ...
            if ((i+1) % this.nCombo == 0) {
                ...
                this.lethalNum += 1;
                // 天候ダメージ
                if (this.weatherDamage && this.weather == 'すなあらし') {
                    let flag = true;
                    let t = p2.currentType();
                    if (['いわ','じめん','はがね'].includes(t[0]) || ['いわ','じめん','はがね'].includes(t.pop())) {
                        flag = false;
                ...
                // グラスフィールド
                if (this.glassRecovery && this.field == 'グラスフィールド' && !p2.isFloat()) {
                ...
                if (p2.item == 'くろいヘドロ' && p2.hasItem) {
                    let r = p2.currentType().includes('どく') ? 1/16 : -1/8;
                ...
                // ねをはる（未実装）
                // 状態異常
                if (this.ailmentDamage && ['どく', 'やけど'].includes(p2.ailment)) {
                    let d;
                    if (p2.ailment == 'どく') {
        ...
        this.totalDamage = offsetKeys(this.totalDamage, totalConstDamage, false);
    }
...
(async () => {
    ...
    for (let i=0; i<moves.length; i++) {
        ...
        battle.weather = 'すなあらし'
        battle.field = 'グラスフィールド'
